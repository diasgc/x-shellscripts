...
if test -z "$INIT"; then
  case "$(uname -s)" in
    Linux)  BUILD_TRIP=$(echo $(uname -m)-linux-gnu)
      ...
      [ -n "$(which getprop)" ] && BUILD_TRIP=$(echo $(uname -m)-linux-android) && \
  ...
  test $bty = 'ac' && test -z $(which autoconf) && \
    aptInstall automake autoconf autogen autopoint libtool m4
  export BUILD_TRIP HOST_NPROC=$(nproc) INIT="ON" ROOTDIR=`dirname "$0"`
fi
